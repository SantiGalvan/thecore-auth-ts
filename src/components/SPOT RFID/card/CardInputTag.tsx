...
const CardInputTag = ({ title, showTag, value, tag, showButton, handleClick }: CardInputTagProps) => {
    return (
        <div className={`border border-slate-300 rounded-xl p-2 m-2 shadow-xl transition-all duration-700 transform ${title === 'TAG B' ? 'delay-200' : 'mb-4'} ${showTag ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
            ...
            <InputGroup
                label={'TID'} placeholder={`Leggi ${title}`}
                id={tag ? `tid-${tag}` : 'tid'} isTag={true} value={value?.valueTID || ''}
            />
            <InputGroup
                label={'EPC'} placeholder={`Leggi ${title}`}
                id={tag ? `epc-${tag}` : 'epc'} isTag={true} value={value?.valueEPC || ''}
            />
            {!showButton && (
