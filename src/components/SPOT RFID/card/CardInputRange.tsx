...
interface CardInputRangeProps {
    minReadingPower: number;
    maxReadingPower: number;
    stepRangePower: number;
    rangeValue: number;
...
const CardInputRange = ({ minReadingPower, maxReadingPower, stepRangePower, rangeValue, setRangeValue, show, handleClick }: CardInputRangeProps) => {
    ...
    return (
        <div className={`border border-slate-300 rounded-xl p-2 m-2 shadow-xl transition-all duration-700 transform ${show ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
            ...
            <div className="relative my-12 w-[90%] mx-auto">
                <input
                    id="power" type="range"
                    min={minReadingPower} max={maxReadingPower} step={stepRangePower}
                    value={rangeValue}
